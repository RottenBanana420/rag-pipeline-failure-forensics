...
**`reciprocal_rank_fusion` converted from `@traced` to manual `with span(...)`, uniquely among the six retrieval/ranking call sites** — `confidence_fn` only ever sees a call's *return* value, but RRF's returned `similarity` is the fused RRF score (`~1/60` per weighted-rank term), not a `[0,1]` quality signal — feeding it through `confidence_from_score` would bottom out at confidence `1` on every call regardless of actual retrieval quality (verified by a regression test: `test_confidence_score_uses_underlying_similarity_not_rrf_score`). The fix reuses `mean_similarity_confidence` but calls it directly, inside the function, on the *pre-fusion* hits (`hits_by_id[cid]`, captured before their `similarity` is overwritten with the RRF score) — mid-function detail no `confidence_fn(result)` hook could reach. This is exactly the "needs mid-function detail" case the `span()`-for-LLM-sites-vs-`traced()`-for-everything-else split already exists for; the manual `input` construction inside `reciprocal_rank_fusion` replicates the argument dict `@traced` used to build (same insertion order, same `default_serialize` call) so no test needed to change.
