...
**Standalone unit, no orchestrator, scoped to span identification only** — Same "directly-callable unit" pattern as `verify_citations`/`score_confidence`/`build_fallback_response`: `find_root_cause_span(trace, judge, threshold)` takes an already-loaded `Trace` and judge instance as plain parameters; no orchestrator yet loads a human-flagged trace and calls this automatically. Failure-type categorization (the six labels from the project spec) and the narrative evidence-chain builder are separate, later tasks — this feature only identifies which span is the root cause.
