...
class TestFixedSizeChunker:
    def test_returns_chunk_objects(self) -> None:
        chunker = Chunker(
            Settings(chunk_strategy="fixed_size", chunk_size=100, chunk_overlap=20)
        )
    ...
    def test_chunk_id_deterministic(self) -> None:
        ...
        assert [c.chunk_id for c in chunker.chunk([_make_doc()])] == [
    ...
    def test_small_doc_is_single_chunk(self) -> None:
        chunker = Chunker(
            Settings(chunk_strategy="fixed_size", chunk_size=10000, chunk_overlap=0)
        )
...
class TestRecursiveHeaderChunker:
    def test_returns_chunks(self) -> None:
        chunker = Chunker(
            Settings(
                chunk_strategy="recursive_header", chunk_size=100, chunk_overlap=20
            )
        )
    ...
    def test_strategy_field_is_recursive_header(self) -> None:
        ...
        assert all(
            c.strategy == "recursive_header" for c in chunker.chunk([_make_doc()])
        )
    ...
    def test_keeps_paragraph_content(self) -> None:
        ...
        chunker = Chunker(
            Settings(chunk_strategy="recursive_header", chunk_size=500, chunk_overlap=0)
        )
    ...
    def test_empty_doc_produces_no_chunks(self) -> None:
        chunker = Chunker(
            Settings(chunk_strategy="recursive_header", chunk_size=100, chunk_overlap=0)
        )
        assert chunker.chunk([_make_doc(text="")]) == []
...
class TestSemanticChunker:
    ...
    def test_splits_at_topic_boundary(self) -> None:
        ...
        with patch("openai.OpenAI") as MockClient:
            MockClient.return_value.embeddings.create.return_value = (
                self._mock_embeddings(vecs)
            )
    ...
    def test_single_sentence_skips_api(self) -> None:
        ...
        chunker = Chunker(
            Settings(chunk_strategy="semantic", chunk_size=1000, chunk_overlap=0)
        )
