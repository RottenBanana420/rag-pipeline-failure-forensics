...
`GENERATION_LLM_MODEL`, `CITATION_JUDGE_MODEL`, `ANSWER_COMPLETENESS_JUDGE_MODEL`, `ROOT_CAUSE_JUDGE_MODEL`, `FAILURE_CATEGORY_JUDGE_MODEL`, and `EVIDENCE_CHAIN_JUDGE_MODEL` all default to `claude-sonnet-4-5` ($3/$15 per MTok in/out). For iterative dev/test runs, set all six to `claude-haiku-4-5` (~5x cheaper on output, the dominant cost) — reserve Sonnet for occasional accuracy-checkpoint runs. Every question asked through the query dashboard now spends on at least three of these six (`GENERATION_LLM_MODEL` once, `CITATION_JUDGE_MODEL` once per citation, `ANSWER_COMPLETENESS_JUDGE_MODEL` once) via `ask_question`, in addition to root-cause/failure-category/evidence-chain judges running once per span/diagnosis during a backward trace walk from the trace view. The evidence-chain judge's prompt payload scales with the full unhealthy tail's input/output text rather than a single span, making it the most token-heavy of the six per call despite running least often. A Claude Pro/Max subscription does **not** cover API usage; API calls need a separate console.anthropic.com key with its own billing.
