...
| Storage | SQLite (trace metadata, `src/tracing/index.py`) + JSON files (full traces, `src/tracing/storage.py`) — implemented; `src/frontend/query_service.py`'s `ask_question` is the first live (non-test) caller wiring these into a per-request pipeline (see Module Layout) |
| API | FastAPI |
| Frontend | Streamlit (`src/frontend/`, implemented) — chosen over React because no API/HTTP layer exists yet; a two-page `st.navigation` app (query dashboard + trace view) calls `ask_question`/`load_trace`/`list_trace_records`/`find_root_cause_span`/etc. directly as Python functions instead of requiring `src/api/main.py` to be built first |
...
# Run the frontend (query dashboard + trace view, two pages via st.navigation)
...
# Run the golden-dataset eval suite (spends real LLM API money — see
# "LLM Judge Cost Management" below) — optional --category/--limit/--output-dir
...
src/
  ...
  generation/         # Grounded prompt (prompts.py), citation_parser.py (regex [N] extraction),
                      # answer_generator.py (AnswerGeneratorProtocol + make_answer_generator factory
                      # — the LLM call that actually produces the [N]-cited answer text)
                      # providers/answer_generator_anthropic.py, providers/answer_generator_openai.py —
                      # same lazy-import factory pattern as the judges, but plain messages.create/
                      # chat.completions.create (free text, not structured output)
  ...
  tracing/            # models.py — Trace/Span pydantic models (PipelineStep/TraceStatus Literals),
                      ...
                      # Trace-per-request orchestrator — src/frontend/query_service.py's ask_question
                      # is the first live caller wrapping collect_spans() + persist_trace() together;
                      # no src/api/ (Phase 7) equivalent exists yet
  ...
  evaluation/         # dataset.py (GoldenCase, load_golden_dataset/filter_cases over
                      # data/golden/qa_dataset.json), corpus.py (ensure_golden_corpus_indexed —
                      # idempotent DocumentLoader/Chunker/Indexer wiring into an isolated
                      # eval_chroma_persist_dir, since the golden corpus is never indexed
                      # elsewhere and scripts/seed_corpus.py remains a stub), retrieval_relevance.py
                      # (score_retrieval_relevance — deterministic recall metric, no LLM),
                      # answer_correctness.py + faithfulness.py (+ providers/answer_correctness_
                      # judge_{anthropic,openai}.py, providers/faithfulness_judge_{anthropic,openai}.py —
                      # two new LLM-as-judge metrics, same lazy-import factory pattern as
                      # make_citation_judge/make_completeness_judge), runner.py (run_test_case/
                      # run_eval/aggregate — wires retrieval+generation+all four metrics per case,
                      # citation accuracy reuses verify_citations directly), regression_tracker.py
                      # (save_report/load_latest_report/compare_reports — one EvalReport JSON per
                      # run under eval_output_dir, flags metric drops > eval_regression_threshold)
  api/                # FastAPI app, route handlers (/ask, /flag, /ingest, /documents)
  frontend/           # Streamlit multipage app (`streamlit run src/frontend/app.py`) — query
                      # dashboard + trace view, via st.navigation/st.Page over app_pages/ (not the
                      # legacy pages/ auto-discovery, which conflicts with it)
                      # query_service.py — ask_question (retrieval -> generation -> citation
                      # verification -> confidence scoring -> fallback gating, inside
                      # collect_spans(), always ending in persist_trace — "success"/"degraded" on
                      # completion, "failure" + re-raise on exception) + build_hybrid_retriever
                      # (wires embedder/vector_store/bm25_store/reranker into a HybridRetriever; no
                      # caching of its own, same rule diagnosis_service.py follows); the second
                      # src/frontend/ module (besides diagnosis_service.py) importing across the
                      # retrieval/generation/tracing module boundaries for one feature
                      ...
                      # + cited_chunk_indices (list[CitationVerificationResult] -> sorted deduplicated
                      # chunk indices, used by the query dashboard's "jump to source" button row)
                      ...
                      # text is untrusted)
                      # flags.py — save_flag/load_flag, persists a human "flag as bad output" review
                      # (confirm the algorithm's diagnosis, or a full re-label override of span +
                      # category + note) as JSON, one file per trace_id holding a single FlagRecord
                      # (data/eval/flags/{trace_id}.json — trace-level, unlike corrections.py's
                      # per-span mapping); DiagnosisSummary flattens a DiagnosisResult into plain
                      # str/int fields for storage, HumanReview always carries a full
                      # span_id/category/note triple even when confirmed=True
                      # app.py — thin st.navigation/st.Page entrypoint: st.set_page_config once,
                      # then hands off to one of the two app_pages/ modules below
                      # app_pages/query_dashboard.py — ask a question (st.form-gated, real LLM
                      # spend), cache the QueryResult in st.session_state keyed by the asked query
                      # text; answer with inline [N] markers + a "jump to source" button row
                      # (cited_chunk_indices) that highlights the matching chunk card; confidence
                      # breakdown via st.metric x4; hybrid-vs-dense-only comparison toggle (one
                      # extra cheap DenseRetriever call, cached per query, run outside
                      # collect_spans()); "View full trace" sets preselect_trace_id and
                      # st.switch_page()s into app_pages/trace_view.py
                      # app_pages/trace_view.py — the engineer-facing forensics page (formerly
                      # app.py's entire body), including the "Flag as bad output" button (available
                      # on every trace, not gated on status) and its confirm/override finalize
                      # actions; reads st.session_state["preselect_trace_id"] (set by the query
                      # dashboard) to default its trace selectbox, then clears it
scripts/
...
tests/
  ...
  unit/generation/    # test_prompts, test_citation_parser, test_citation_verifier,
                      # test_confidence_scorer, test_fallback_response, test_answer_generator
                      # providers/         # per-judge-provider tests (citation/completeness × anthropic/openai),
                      # plus test_answer_generator_anthropic/test_answer_generator_openai
  ...
  unit/frontend/      # test_view_models (color-coding + view-model construction, including
                      # build_span_diff_view_model's diff-segment tagging and cited_chunk_indices),
                      # test_diagnosis_service (fake judges, no real API calls), test_query_service
                      # (fake retriever/generator/judges — success/degraded/failure trace paths,
                      # no real API calls), test_corrections (save/load round-trip via tmp_path),
                      # test_flags (save/load round-trip + diagnosis-summary flattening, also via
  ...
  unit/evaluation/    # test_golden_dataset.py — structural integrity checks on
                      ...
                      # guards the data artifact itself against drift as entries are hand-edited
                      # or, later, auto-appended from confirmed flags (Phase 6 item 4)
                      # test_dataset, test_corpus, test_retrieval_relevance (pure function,
                      # explicit Path(source_path).name mismatch case), test_faithfulness,
                      # test_answer_correctness, test_runner (fake retriever/generator/judges,
                      # per-case error isolation, None-exclusion in aggregate),
                      # test_regression_tracker — src/evaluation/'s own test coverage
                      # providers/         # per-judge-provider tests (faithfulness/answer_correctness
                      # × anthropic/openai), same shape as generation/analysis provider tests
...
**Provider abstraction:** Embedding and vector-store backends are chosen at runtime via `EMBEDDING_PROVIDER`/`VECTOR_STORE_PROVIDER` env vars, resolved through `make_embedder`/`make_vector_store` factories against `EmbedderProtocol`/`VectorStoreProtocol`. Provider SDKs are imported lazily inside the factory (not at module level) so installing one provider's extra doesn't require the others. On first write, `ChromaVectorStore` stamps the collection metadata with the embedder's `provider_id` and dimension count; on later opens it refuses to proceed if the configured provider doesn't match, instead of silently corrupting the index. `pyproject.toml` has no dedicated `llm-openai` extra: setting any `*_JUDGE_PROVIDER=openai` or `GENERATION_LLM_PROVIDER=openai` needs the `openai` package, which is installed via the `embed-openai` extra (`pip install -e ".[dev,embed-openai]"`) — the same package the OpenAI embedder provider uses, reused rather than duplicated. Installing only `dev`/`llm-anthropic` and then setting an `openai` provider fails with an `ImportError` at first use, not a clear configuration error, since the lazy-import factories don't validate installed extras up front.
...
**Trace view (frontend):** `src/frontend/app_pages/trace_view.py` (`streamlit run src/frontend/app.py`, one of two pages in the `st.navigation` app — see "Query dashboard (frontend)" below for the other and for why `app.py` itself is now just a thin navigation entry) is a Streamlit page rendering a `Trace`'s spans as a left-to-right flow diagram, color-coded green/yellow/red, with a click-through node-detail panel. Streamlit was chosen over React specifically because no API/HTTP layer exists yet (`src/api/` is still a Phase 7 placeholder) — the app calls `load_trace`/`list_trace_records`/`find_root_cause_span`/`categorize_failure`/`build_evidence_chain` directly as Python functions, versus React which would require standing up `src/api/main.py` from scratch first, turning "build a trace view" into "build Phase 5 and Phase 7 at once." `Trace.spans` is a flat, execution-order list with no parent/child field, and multiple spans can share the same `step` (e.g. `HybridRetriever`'s dense/sparse retrieval legs are both `step="retrieval"`), so `build_graph_view_model` (`src/frontend/view_models.py`) makes **each span its own node**, positioned in `trace.spans` order and connected by sequential edges — not one node per distinct step name. `node_status` colors a node red if it matches the current `RootCauseDiagnosis.root_cause_span.span_id` (always wins, regardless of `confidence_score`), else yellow if `Span.confidence_score <= settings.root_cause_quality_threshold` (reusing that setting's existing 1-5 "at or below is unreasonable" semantics rather than inventing a parallel threshold), else green — this needs no LLM call, so opening any trace is free. Root-cause coloring only appears after an explicit "Flag as bad output" button click (renamed from "Diagnose root cause" when the flagging interface repurposed it — see below), which calls `run_diagnosis` (`src/frontend/diagnosis_service.py`, the only `src/frontend/` module importing from `src/analysis/`) — real per-span LLM judge spend, per the "LLM Judge Cost Management" section below, so it must be deliberate, not automatic on trace load. The resulting `DiagnosisResult` is cached in `st.session_state` keyed by `trace_id`, so revisiting the same trace within a session doesn't re-spend; a persisted flag (see below) also seeds this coloring across sessions. The node graph is rendered via `streamlit_flow` (`streamlit-flow-component`, a new `frontend` extra in `pyproject.toml`), chosen via Context7 lookup over `streamlit-agraph` (undocumented there — a maintenance-signal red flag); its `get_node_on_click`/`StreamlitFlowState.selected_id` click detection, per-node `style` dict, and `ManualLayout` positioning map directly onto this flat/ordered span-list model. `Span` has no embeddings/vector field, so `detail_panel.py` shows an explicit "embeddings not captured in this trace" note for retrieval/ranking-step spans rather than extending `Span` — that would mean touching tracing instrumentation across every retrieval/ranking provider and storing potentially large vectors in every trace JSON, out of scope for a UI-only task. See `docs/DECISIONS.md` for the streamlit_flow `key=`/`st.session_state` collision gotcha hit during implementation.

**Diff view (frontend):** `docs/PROJECT_SPEC.md` (Phase 5, item 2) asks for a per-span "received vs. produced vs. should have produced" comparison on failed traces, with the divergence highlighted. At the time this feature was built, no golden-dataset or human-correction data source existed yet — `src/evaluation/` was still a Phase 6 placeholder and no `Span`/`Trace` field carries an expected output — so `src/frontend/corrections.py` introduces the "human correction" half of that spec sentence only: `save_correction`/`load_correction` persist a human-typed expected output per `(trace_id, span_id)` as one JSON file per trace under `settings.human_corrections_dir` (default `./data/eval/corrections`), mirroring `src.tracing.storage`'s one-file-per-id convention. This diff view still only reads from `corrections.py`, not from `src/evaluation/`'s now-implemented golden dataset/eval metrics (Phase 6, items 1–2) — wiring the diff view to golden `expected_answer`s remains unbuilt. `build_span_diff_view_model` (`src/frontend/view_models.py`) computes the divergence with a word-level diff (`difflib.SequenceMatcher` over whitespace-preserving tokens, the same technique `difflib.HtmlDiff`/`git diff --word-diff` use — no new dependency), returning `None` segments when no correction has been entered yet rather than diffing against an empty string. `diff_panel.py` builds this from the expected-output text area's *live* return value, not the on-disk saved correction, so the highlighted divergence updates immediately as the user types/blurs rather than requiring a save first; "Save correction" (`st.toast("Correction saved.", icon="✅")` on click, no `st.rerun()`) is purely for persistence. It renders the three columns and, only for `trace.status != "success"` traces (matching the spec's "for failed traces" — this gate is now diff-view-specific, since the flagging interface's own button below deliberately dropped the equivalent gate it used to share), the highlighted segments as inline-styled HTML via `st.html()` (not `st.markdown(unsafe_allow_html=True)` — that would run the diff text through Markdown parsing first, corrupting literal `*`/`_` characters), wrapped in a `white-space:pre-wrap` container so whitespace-only divergence doesn't visually collapse. Every segment's text is passed through `html.escape()` first, since `Span.output` and human corrections are both untrusted text and neither `st.markdown(unsafe_allow_html=True)` nor `st.html` escapes automatically (confirmed via Context7 lookup against Streamlit's docs); `detail_panel.py` avoids this risk entirely by using `st.text_area`, which auto-escapes, but the diff view needs per-word span-level styling that a plain text widget can't produce. `st.html` predates the project's prior `streamlit>=1.38` floor, so `pyproject.toml`'s `frontend` extra now requires `streamlit>=1.41`.

**Flagging interface (frontend):** `docs/PROJECT_SPEC.md` (Phase 5, item 3) asks for a button that lets a user mark any trace as "bad output," runs the backward trace analysis, displays the diagnosis, and lets the user confirm or override it — "the feedback signal for your eval loop." This repurposes the diff view's existing "Diagnose root cause" button into "Flag as bad output" rather than adding a second control alongside it: one action both runs `run_diagnosis` (unchanged — still session-cached, still real Anthropic/OpenAI spend gated behind an explicit click) and enters the flag/review flow. The button's old `trace.status == "success"` gate is removed entirely: the spec says "mark *any* trace," including ones the pipeline itself scored `"success"`, since a human can still catch a bad output the automated checks missed. This is a deliberate contrast with the diff view, which stays gated on `trace.status != "success"` — a different spec sentence for a different feature, not an inconsistency. "Confirm or override" is implemented as a full re-label, not accept/reject: `src/frontend/flags.py`'s `HumanReview` (`confirmed`, `span_id`, `category`, `note`) always carries a complete verdict, even when the human clicks "Confirm diagnosis" (the algorithm's own span/category are copied across verbatim) — so a downstream reader never has to branch on `confirmed` to find the human's actual answer. Overriding uses an `st.form` (batches the span/category/note widgets into one submit-triggered rerun) with a root-cause-span selectbox pre-populated from `trace.spans` and a failure-category selectbox from `get_args(FailureCategory)`; it's the only finalize path when the diagnosis run found no root cause (`summary is None`), since there's nothing to confirm in that case. Persistence — `save_flag`/`load_flag`, one JSON file per trace holding a single `FlagRecord` (trace-level, unlike `corrections.py`'s per-span mapping), hand-rolled via `dataclasses.asdict` (no `dacite`/`cattrs`/`dataclasses-json` dependency, mirroring `corrections.py`'s own hand-rolled shape) — happens **only** on "Confirm diagnosis" or "Save override," never from running the diagnosis alone, mirroring `corrections.py`'s "button click persists, nothing else does" rule. `DiagnosisSummary` flattens a `DiagnosisResult` into plain str/int fields (same rationale as `EvidenceEntry`) so the stored JSON never embeds live `Span`/pydantic objects. A trace with an existing `FlagRecord` shows a compact persisted summary plus a "Redo review" button instead of the full flow every time; `app_pages/trace_view.py` also falls back to a persisted flag's `diagnosis.root_cause_span_id` for red-node graph coloring when no session-state diagnosis exists, so revisiting an already-flagged trace in a fresh session still shows the root cause without re-spending LLM calls. The `st.rerun()` after Confirm/Override is a real state transition (closing the review, reloading the just-written flag from disk), not the avoidable-rerun-on-every-keystroke pattern the diff view's `docs/DECISIONS.md` entry warns against. `DiagnosisSummary`/`HumanReview`'s field choices anticipate but do not build Phase 6 item 4 (auto-generating eval cases from confirmed flags) — same "persist the human-input half now, leave auto-generation for later" scoping the diff view's corrections already established.

**Query dashboard (frontend):** `docs/PROJECT_SPEC.md` (Phase 5, item 4) asks for a dashboard where a user asks a question and sees the generated answer with clickable citations, ranked retrieved chunks, a per-dimension confidence breakdown, and a hybrid-vs-dense-only comparison toggle. Unlike every other Phase 5 item, this required building the missing pipeline first, not just a UI: no module called an LLM to produce the actual `[N]`-cited answer text (`prompts.py` only builds the prompt), and no orchestrator wired retrieval → generation → citation verification → confidence scoring → tracing together for a live request. `src/generation/answer_generator.py` (`AnswerGeneratorProtocol` + `make_answer_generator`) fills the first gap, same lazy-import factory pattern as `make_citation_judge`/`make_completeness_judge`, but using each SDK's plain `messages.create`/`chat.completions.create` rather than `.parse`/structured output — an answer is free text, not a verdict. `src/frontend/query_service.py`'s `ask_question(query, retriever, settings)` fills the second, mirroring `diagnosis_service.py`'s "thin per-feature orchestrator, the only other module reaching across `src/frontend/`'s module boundary" pattern: it runs the full pipeline inside `collect_spans()`, then always calls `persist_trace` — `"success"`/`"degraded"` (fallback fired) on completion, or `"failure"` (with whatever spans completed) on any exception, re-raised afterward so the page can show `st.error()` referencing the still-inspectable `trace_id`. This makes the query dashboard the first live (non-test) caller of `persist_trace`. Unlike root-cause diagnosis, confidence scoring is **not** gated behind a second click — it's the dashboard's core displayed output, not an optional deep-dive, so it runs automatically on every `ask_question` call. When `build_fallback_response` fires, the UI shows the fallback message instead of the generated answer, but `QueryResult.answer_text`/`citation_results`/`confidence` are computed and persisted regardless, so engineers can see what the model would have answered. The hybrid-vs-dense-only toggle only triggers one extra, cheap `DenseRetriever.retrieve()` call for a side-by-side chunk list (no second generation call or confidence score), cached per query in `st.session_state`, and run outside `collect_spans()` — a UI exploration aid, not part of the canonical persisted trace. Adding a second, genuinely different screen justified real navigation: `src/frontend/app.py` became a thin `st.navigation`/`st.Page` entry (`app_pages/`, not the legacy `pages/` auto-discovery it conflicts with) while the former single-page trace view body moved to `app_pages/trace_view.py` unchanged aside from removing its own now-illegal second `st.set_page_config` call and reading `st.session_state["preselect_trace_id"]` (set by the dashboard's "View full trace" button before `st.switch_page`) to default its trace selectbox. `streamlit run src/frontend/app.py` is unchanged. Citations are rendered as plain `[N]` markers with a row of "jump to source" buttons below (`cited_chunk_indices`, `view_models.py`) rather than true anchor-scroll — Streamlit has no in-page DOM scroll-to primitive — a deliberate, pragmatic reading of "clickable citations linking to source chunk." See `docs/DECISIONS.md` (2026-07-15 entry) for the full design rationale.

**Automated eval metrics:** `docs/PROJECT_SPEC.md` (Phase 6, item 2) asks for four metrics per golden test case — answer correctness, faithfulness, retrieval relevance, citation accuracy — run on demand and tracked over time to spot regressions. Citation accuracy already existed (`verify_citations`); the other three are new. `src/evaluation/answer_correctness.py` (+ `providers/answer_correctness_judge_{anthropic,openai}.py`) is a boolean `CorrectnessVerdict(correct, reasoning)` judge, matching every other judge's convention rather than `StepQualityVerdict`'s graded scale — factual correctness against a known golden answer is naturally binary, and a 1-5 scale would add prompt ambiguity with no benefit to tracking a fraction-correct rate over time. `src/evaluation/faithfulness.py` (+ its two providers) decomposes an answer into claims via a naive sentence split (`split_into_claims` — no `[N]` marker to anchor on, unlike `citation_parser`) and judges each against the *full* concatenated context, not just whichever chunk a citation happened to reference, so a hallucinated-but-uncited claim is still caught; a single holistic call was rejected because it would hide exactly which claim failed. `src/evaluation/retrieval_relevance.py` is the one metric that's deterministic, not judge-backed: it compares retrieved hits' `(source_path, section_heading)` against the golden dataset's `source_documents`/`source_sections` (confirmed parallel arrays, paired by index) — `Path(hit.source_path).name` is required, not raw string equality, since `source_path` is a full path (`str(Path)`) while the dataset stores bare filenames. `src/evaluation/corpus.py`'s `ensure_golden_corpus_indexed` fixes a gap found during implementation — the golden corpus was never indexed anywhere — by wiring the existing `DocumentLoader`/`Chunker`/`Indexer` into a new isolated `eval_chroma_persist_dir` (never production `CHROMA_PERSIST_DIR`), idempotent via `vector_store.count() > 0`. `src/evaluation/runner.py`'s `run_test_case`/`run_eval` are deliberately **not** wrapped in `collect_spans()`/`persist_trace` — a 51-case batch run persisting 51 synthetic traces would conflate with live request traces in the trace-view UI; judge providers' `span()` calls are harmless no-ops without an active sink. `regression_tracker.py` persists one `EvalReport` JSON per run (`eval_output_dir`) and flags any metric dropping by more than `eval_regression_threshold` against the immediately-previous run. `scripts/run_eval.py` is the CLI entry point. `score_faithfulness` also short-circuits to `score=None` when `answer_text` is exactly the model's canonical `INSUFFICIENT_CONTEXT_RESPONSE` fallback string (found during post-implementation review) — without it, a `no_answer`-category answer correctly refusing to guess is not a factual claim and could be scored unfaithful by a strict judge, exactly backwards for that category. See `docs/DECISIONS.md` (2026-07-15 and 2026-07-16 entries) for the full design rationale.

**Feedback loop:** Human flagging is implemented (`src/frontend/flags.py`, above) — on-demand and human-triggered via the "Flag as bad output" button, not automatic. Confirmed or overridden diagnoses are persisted as a `FlagRecord`, but nothing yet reads those records to auto-generate new eval test cases or grow `src/evaluation/`'s dataset — that half of `docs/PROJECT_SPEC.md`'s Phase 6 item 4 remains unbuilt.
...
FLAGGED_TRACES_DIR=    # Directory for per-trace "bad output" flag records used by the trace view's
                        # flagging interface (default: ./data/eval/flags)
...
# Answer generation (the grounded LLM call that produces the [N]-cited answer text)
GENERATION_LLM_PROVIDER=    # anthropic | openai (default: anthropic)
GENERATION_LLM_MODEL=       # Model name (default: claude-sonnet-4-5 for anthropic; gpt-4o-2024-08-06 for openai)
GENERATION_LLM_TEMPERATURE= # Sampling temperature for the generation call, 0.0-1.0 (default: 0.0)
...
# Answer correctness (LLM-as-judge comparing a generated answer against the
# golden dataset's hand-written expected_answer)
ANSWER_CORRECTNESS_JUDGE_PROVIDER=    # anthropic | openai (default: anthropic)
ANSWER_CORRECTNESS_JUDGE_MODEL=       # Model name (default: claude-sonnet-4-5 for anthropic; gpt-4o-2024-08-06 for openai)
ANSWER_CORRECTNESS_JUDGE_TEMPERATURE= # Sampling temperature for the judge call, 0.0-1.0 (default: 0.0)

# Faithfulness (LLM-as-judge checking every claim in a generated answer is
# grounded in the full retrieved context, not just cited claims)
FAITHFULNESS_JUDGE_PROVIDER=    # anthropic | openai (default: anthropic)
FAITHFULNESS_JUDGE_MODEL=       # Model name (default: claude-sonnet-4-5 for anthropic; gpt-4o-2024-08-06 for openai)
FAITHFULNESS_JUDGE_TEMPERATURE= # Sampling temperature for the judge call, 0.0-1.0 (default: 0.0)

# Eval harness (golden-dataset metric runs — scripts/run_eval.py)
EVAL_OUTPUT_DIR=            # Directory for one EvalReport JSON per run (default: ./data/eval/runs)
EVAL_CHROMA_PERSIST_DIR=    # Isolated Chroma dir the eval runner indexes the golden corpus into —
                             # never production CHROMA_PERSIST_DIR (default: ./data/eval/chroma)
EVAL_REGRESSION_THRESHOLD=  # A metric (overall or per-category) dropping by more than this vs. the
                             # previous run is flagged as a regression (default: 0.05)
```

## LLM Judge Cost Management

`GENERATION_LLM_MODEL`, `CITATION_JUDGE_MODEL`, `ANSWER_COMPLETENESS_JUDGE_MODEL`, `ROOT_CAUSE_JUDGE_MODEL`, `FAILURE_CATEGORY_JUDGE_MODEL`, `EVIDENCE_CHAIN_JUDGE_MODEL`, `ANSWER_CORRECTNESS_JUDGE_MODEL`, and `FAITHFULNESS_JUDGE_MODEL` all default to `claude-sonnet-4-5` ($3/$15 per MTok in/out). For iterative dev/test runs, set all eight to `claude-haiku-4-5` (~5x cheaper on output, the dominant cost) — reserve Sonnet for occasional accuracy-checkpoint runs. Every question asked through the query dashboard now spends on at least three of these eight (`GENERATION_LLM_MODEL` once, `CITATION_JUDGE_MODEL` once per citation, `ANSWER_COMPLETENESS_JUDGE_MODEL` once) via `ask_question`, in addition to root-cause/failure-category/evidence-chain judges running once per span/diagnosis during a backward trace walk from the trace view. `python scripts/run_eval.py` spends on `GENERATION_LLM_MODEL` plus all four eval judges once per golden case (`CITATION_JUDGE_MODEL` once per citation, `FAITHFULNESS_JUDGE_MODEL` once per claim, `ANSWER_CORRECTNESS_JUDGE_MODEL` once) — the priciest single command in the repo to run at full scale (51 cases), since it's the only one that fans out real spend across the entire golden dataset in one invocation rather than one live question at a time; `--limit N`/`--category` keep a dev-loop run cheap. The evidence-chain judge's prompt payload scales with the full unhealthy tail's input/output text rather than a single span, making it the most token-heavy of the eight per call despite running least often. A Claude Pro/Max subscription does **not** cover API usage; API calls need a separate console.anthropic.com key with its own billing.

**Testing cadence on real data:** run a small batch (10–20 questions, or `python scripts/run_eval.py --limit 10`) any time `generation/`/`evaluation/` changes or a new orchestrator step is wired in; run a fuller batch (100+ questions, or the full `python scripts/run_eval.py` with no `--limit`) once per phase completion (end of Phase 2, 3, 4, 6). There's no CI spending real API money, so these are manual checkpoints, not per-commit gates.
