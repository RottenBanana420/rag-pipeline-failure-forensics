...
src/
  ...
  frontend/           # Streamlit trace view (`streamlit run src/frontend/app.py`)
                      ...
                      # app.py — Streamlit entrypoint wiring the above together, including the
                      # "Flag as bad output" button (available on every trace, not gated on status)
                      # and its confirm/override finalize actions
scripts/
...
tests/
  ...
  unit/frontend/      # test_view_models (color-coding + view-model construction, including
                      ...
                      # tmp_path), test_flags (save/load round-trip + diagnosis-summary flattening,
                      # also via tmp_path, no mocks) — graph_render.py/detail_panel.py/diff_panel.py/
                      # app.py are Streamlit UI and verified manually instead
