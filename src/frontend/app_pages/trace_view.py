"""Engineer-facing trace view page.
...
`st.session_state["preselect_trace_id"]` (set by the query dashboard's "View
full trace" button) defaults the sidebar trace selectbox to that trace, then
is cleared so a later manual selection isn't overridden on rerun.
"""
...
preselect_trace_id = st.session_state.pop("preselect_trace_id", None)
...
with st.sidebar:
    ...
    default_index = 0
    if preselect_trace_id is not None:
        default_index = next(
            (i for i, r in enumerate(records) if r.trace_id == preselect_trace_id),
            0,
        )
    selected_record = st.selectbox(
        "Trace", records, index=default_index, format_func=_format_record
    )
